...
const FiltersDropdown = ({ categories, selectedCategories, handleSelectCategory, showCategories, setShowCategores }: FiltersDropdownProps) => {
    ...
    return (
        <div className="bg-white shadow absolute top-full left-0 overflow-y-hidden duration-150" style={{ width: `${150}px`, maxHeight: showCategories ? '200px' : 0 }} ref={categoriesBox}>
            {
                categories.map((category: string) => <div key={category} className={`w-full px-2 py-2 hover:bg-accent cursor-pointer capitalize ${selectedCategories?.split(',').includes(category) ? 'bg-secondary' : 'bg-white'}`} onClick={() => handleSelectCategory(category)}>{category}</div>)
            }
        </div>
    )
}
